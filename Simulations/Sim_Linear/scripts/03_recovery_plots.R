# Figures for the recovery study (greyscale):
...
#   sigma_vs_window.png - midpoint sigma error grows with vaguer dating
...
recovery_plot <- ggplot(estimates_vs_truth,
  ...
  labs(x = "True value", y = "Estimated (posterior median)",
       ...
       subtitle = "Points on the dashed 1:1 line = recovered. Midpoint sigma sits above the line (overestimated)") +
  panel_theme
...
coverage_rates <- recovery_results %>%
  group_by(model) %>%
  summarise(across(c(intercept_cov50, slope_cov50, sigma_cov50,
                     intercept_cov90, slope_cov90, sigma_cov90), mean),
            .groups = "drop") %>%
...
# Greyscale precision bands behind the data; points coloured by model; per-100yr
# binned mean +/- 95% CI whisker for each model (instead of a loose trend line).
...
binned_summary <- sigma_by_window %>%
...
y_limits <- quantile(sigma_by_window$sigma_err, c(0.004, 0.996))
label_y  <- y_limits[1] + 0.04 * diff(y_limits)
...
sigma_window_plot <- ggplot() +
  ...
  geom_errorbar(data = binned_summary,
                aes(x = x, ymin = q25, ymax = q75, colour = model),
                width = 16, linewidth = 0.7) +
  geom_point(data = binned_summary, aes(x = x, y = median_error, colour = model,
                                        shape = model), size = 2) +
...
# recovery_table.csv : recovery rate (% of datasets where the interval caught the
