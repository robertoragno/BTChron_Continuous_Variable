...
# Set a path where to save figures and a theme for them.
...
#' Periodisation priors, matching 02_recovery_study.R. K phases with Dirichlet
#' concentration alpha_conc; alpha_conc leans low (Beta over [0.1, 10]) so coarser
#' phases are common. Defined once here as functions so I don't forget when I reuse
#' them (and they are functions so it's not a static stored value)
draw_K          <- function(n = 1) sample(3:10, n, replace = TRUE)
draw_alpha_conc <- function(n = 1) 10^(-1 + 2 * rbeta(n, 2, 3.5))

# List of priors to generate a dataset. N is drawn from the same four values as in the recovery study.
draw_prior <- function() list(
  intercept  = runif(1, 2, 15),
  slope      = runif(1, -0.03, 0.03),
  sigma      = runif(1, 0.5, 4),
  K          = draw_K(),
  alpha_conc = draw_alpha_conc(),
  N          = sample(c(50, 100, 200, 400), 1)
)

# Shannon entropy of phases H (~ dating resolution) implied by the K and alpha_conc priors.
# The function partition_timeline() is in simulate.R and is used to generate the phases and 
# their weights, from which H is calculated.
n_draws <- 5000
K          <- draw_K(n_draws)
...
#' draw full parameter sets, simulate, and look at the data generated. 
#' The pooled value distribution flags whether the priors imply plausible measurements; 
#' the panels show six datasets with their true trend.
n_sets <- 500
priors <- replicate(n_sets, draw_prior(), simplify = FALSE)
sims   <- lapply(priors, function(p)
  simulate_linear(p$N, p$intercept, p$slope, p$sigma, p$K, p$alpha_conc))
...
# Show the first six sets as example panels. One label per set, reused for both
# the points and the trend line so they share a facet.
...
# The noise-free trend each set was generated from. Since a line needs only two
# endpoints, we evaluate it at the timespan ends join them with geom_line
trends <- do.call(rbind, lapply(1:6, function(i) {
  p <- priors[[i]]
  data.frame(panel = panel_of(i), Year = c(TMIN, TMAX),
             Value = p$intercept + p$slope * c(TMIN, TMAX))
}))
...
dataset_plot <- ggplot(ex, aes((Start_date + End_date) / 2, Value)) +
  ...
  geom_line(data = trends, aes(Year, Value), colour = "#780000", linewidth = 0.6) +
