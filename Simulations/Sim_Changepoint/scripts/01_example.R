...
fit_latent <- cmdstan_model(here("Simulations", "Sim_Changepoint", "models",
                                 "sim_changepoint.stan"))$sample(
...
fit_midpoint <- cmdstan_model(here("Simulations", "Sim_Changepoint", "models",
                                   "sim_changepoint_midpoint.stan"))$sample(
  data = stan_data, chains = 4, parallel_chains = 4, iter_warmup = 1000,
  iter_sampling = 1000, seed = 42, adapt_delta = 0.95, max_treedepth = 12,
  refresh = 0, show_messages = FALSE)
...
param_levels <- c("Baseline", "Slope 1", "Slope 2", "CP", "Sigma")

parameter_posteriors <- function(fit, model_label) {
  draws <- fit$draws(format = "df")
  tibble(Baseline = draws$baseline_original,
...
true_value_lines <- tibble(
  Parameter = factor(param_levels, param_levels),
  True = c(baseline, slope_1, slope_2, cp, sigma))
...
cat("Wrote exploratory_panel.png, model_comparison_single_fit.png, individual_date_posteriors.png\n")
