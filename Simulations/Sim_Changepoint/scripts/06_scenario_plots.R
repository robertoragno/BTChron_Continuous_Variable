...
#   scenario_skew_bias.png          - slope and changepoint bias vs skew
#                                     strength, faceted by phase width, with
#                                     no width~position correlation assumed
#                                     (the general result)
...
# fitted slope or changepoint, so this figure is deliberately restricted to
# the three cases where within-phase deposition is uniform -- the skew cases
# belong to scenario_skew_bias.png instead, where bias is the point.
...
accuracy <- results %>%
  ...
  pivot_longer(c(baseline_cov90, slope1_cov90, slope2_cov90, cp_cov90, sigma_cov90),
...
precision <- results %>%
  filter(case %in% baseline_cases) %>%
  pivot_longer(c(baseline_width90, slope1_width90, slope2_width90, cp_width90,
                 sigma_width90),
...
# still covers (EIV slopes) reads as the win it is. Same layout and palette as
...
#' scenario_skew_bias.png : Within-phase deposition skew is the only thing
#' that can bias the fitted slopes or shift the changepoint; skew = "none" is
#' the fine/coarse cases themselves (skew_shape = 1, uniform), so the same
#' runs used for the accuracy/precision figure double as the zero-skew anchor
#' here. Slope bias is summarised as the ratio of estimated to true slope
#' (median over reps, with the 25/75% quantile band), same convention as the
...
skew_bias <- results %>%
  ...
  mutate(model = label_model(model),
         ...
         slope1_ratio = slope1_med / true_slope_1,
         slope2_ratio = slope2_med / true_slope_2) %>%
  pivot_longer(c(slope1_ratio, slope2_ratio), names_to = "parameter",
               values_to = "ratio") %>%
  mutate(parameter = factor(parameter, c("slope1_ratio", "slope2_ratio"),
                            c("Slope 1", "Slope 2"))) %>%
  group_by(width, skew, model, parameter) %>%
  summarise(median_ratio = median(ratio),
            lower = quantile(ratio, 0.25),
            upper = quantile(ratio, 0.75), .groups = "drop")
...
cp_bias <- results %>%
  ...
  mutate(model = label_model(model),
         width = label_skew_width(case),
         skew  = label_skew_level(case),
         cp_err = cp_med - true_cp) %>%
  group_by(width, skew, model) %>%
  summarise(median_err = median(cp_err),
            lower = quantile(cp_err, 0.25),
            upper = quantile(cp_err, 0.75), .groups = "drop")

cp_bias_plot <- ggplot(cp_bias, aes(skew, median_err, colour = model,
                                    shape = model, group = model)) +
...
# before trusting a trend or changepoint estimate from either model.
...
position_slope <- position_bias %>%
  ...
  mutate(parameter = factor(parameter, c("slope1_ratio", "slope2_ratio"),
                            c("Slope 1", "Slope 2"))) %>%
  group_by(position, model, parameter) %>%
  summarise(median_ratio = median(ratio),
            lower = quantile(ratio, 0.25),
            upper = quantile(ratio, 0.75), .groups = "drop")

position_cp <- position_bias %>%
...
position_cp_plot <- ggplot(position_cp, aes(position, median_err,
                                            colour = model, shape = model)) +
  geom_hline(aes(yintercept = 0, linetype = "Unbiased"), colour = "grey40") +
  geom_errorbar(aes(ymin = lower, ymax = upper), position = position_dodge(0.4),
                width = 0.15, linewidth = 0.5) +
...
position_bias_plot <- (position_slope_plot | position_cp_plot) +
  plot_layout(guides = "collect") +
  plot_annotation(
    title = "Worked example: bias when phase width correlates with position",
...
scenario_table <- results %>%
  ...
  summarise(H = round(mean(H), 2), sampled_width = round(mean(mean_width)),
            slope1_cov90 = round(100 * mean(slope1_cov90), 1),
            slope1_width90 = round(mean(slope1_width90), 4),
            slope1_ratio = round(median(slope1_med / true_slope_1), 2),
            slope2_cov90 = round(100 * mean(slope2_cov90), 1),
            slope2_width90 = round(mean(slope2_width90), 4),
            slope2_ratio = round(median(slope2_med / true_slope_2), 2),
            cp_cov90 = round(100 * mean(cp_cov90), 1),
            cp_width90 = round(mean(cp_width90), 1),
            cp_err = round(median(cp_med - true_cp), 1),
...
write_csv(scenario_table,
          here("Simulations", "Sim_Changepoint", "output", "scenario_table.csv"))
