...
fit <- readRDS(here("Simulations", "Sim_Changepoint", "output",
                    "fit_changepoint.rds"))
...
mu_cols  <- paste0("mu_pred[", 1:N_pred, "]")
...
p_trend <- ggplot(trend_summary) +
  ...
  geom_rug(data = sim_data, aes(y = Value),
           sides = "r", colour = "grey50", alpha = 0.3,
           length = unit(3, "pt")) +
  scale_fill_manual(
    name   = NULL,
...
ggsave(
  here("Simulations", "Sim_Changepoint", "figures", "model_results_panel.png"),
  p, width = 12, height = 9, dpi = 300, bg = "white"
)

cat("Saved to",
    here("Simulations", "Sim_Changepoint", "figures",
         "model_results_panel.png"), "\n")
