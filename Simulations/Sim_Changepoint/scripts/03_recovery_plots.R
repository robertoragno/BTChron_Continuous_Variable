...
# Also writes recovery_table.csv (accuracy rates) and recovery_width_summary.csv
# for the paper's summary tables.
#
# Five other cuts of this same H-resolved accuracy/precision relationship
# (recovery.png, accuracy.png, precision_vs_entropy.png, sigma_vs_entropy.png,
# accuracy_vs_entropy.png, precision_boxplots.png) were dropped from the main
# script on 2026-07-16 as redundant with the composite above (mirrors the
# Sim_Linear trim of 2026-07-15); the code that produces them still lives in
...
# of 06 sit on this continuum (mean H per case). The skewed cases are left out
...
scenario_table_file <- here("Simulations", "Sim_Changepoint", "output",
                            "scenario_table.csv")
...
width_pairs <- recovery_results %>%
  select(dataset_id, model, baseline_width90, slope1_width90, slope2_width90,
         cp_width90, sigma_width90) %>%
...
width_summary <- width_pairs %>%
  ...
  summarise(median_eiv = median(latent), median_midpoint = median(midpoint),
            ...
            .groups = "drop") %>%
  mutate(parameter = factor(parameter, c("baseline", "slope1", "slope2", "cp", "sigma"),
                            c("Baseline", "Slope 1", "Slope 2", "Changepoint", "Sigma"))) %>%
  arrange(parameter)

write_csv(width_summary,
          here("Simulations", "Sim_Changepoint", "output", "recovery_width_summary.csv"))
