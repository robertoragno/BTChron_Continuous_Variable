#' Purpose: Simulation-based calibration (SBC) for both the latent-date and
#'          midpoint HSGP models. Re-draw true dates and observation noise K times,
#'          refit each model, and check posterior coverage of sigma. Sigma is the
#'          cleanest calibration target because it has a known true value (2.5);
#'          the midpoint model systematically inflates it by absorbing unmodelled
#'          date uncertainty into the residuals.
#'
...
if (file.exists(latent_csv)) {
  ...
  model_latent <- cmdstan_model(here("Simulations", "Sim_GP", "models",
                                     "sim_hsgp.stan"))
  ...
  for (k in seq_len(K)) {
    ...
    fit <- model_latent$sample(
      ...
      iter_warmup = 1000, iter_sampling = 1000, seed = k,
...
if (file.exists(midpoint_csv)) {
  cat("Loading existing midpoint SBC results.\n")
  results_midpoint <- read_csv(midpoint_csv, show_col_types = FALSE)
} else {
  cat("Running midpoint SBC loop...\n")
  model_midpoint <- cmdstan_model(here("Simulations", "Sim_GP", "models",
                                       "sim_hsgp_midpoint.stan"))
  ...
  write_csv(results_midpoint, midpoint_csv)
}
...
rank_df <- bind_rows(
  tibble(sigma = results_latent$sigma_rank,   Model = "Latent-date"),
  tibble(sigma = results_midpoint$sigma_rank, Model = "Midpoint")
) %>%
  pivot_longer(sigma, names_to = "Parameter", values_to = "Rank") %>%
...
p <- ggplot(rank_df, aes(x = Rank)) +
  ...
  labs(
    title = "Posterior rank histogram — sigma (true = 2.5)",
