# End-to-end Case 1: simulate radiocarbon dates, calibrate, fit the marginal
# model on the calibrated posteriors.
#
# This is the first time the model sees a peaked, sometimes multimodal weight
# row rather than a flat one, so it tests what the other two checks cannot:
# whether "one Stan file for all four cases" actually holds. The point-date median
# model is fitted alongside as a reference, not as a result - one dataset says
# nothing about bias, that is the recovery study's job.
