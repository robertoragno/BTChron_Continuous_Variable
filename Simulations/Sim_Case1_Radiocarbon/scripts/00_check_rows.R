# Checks that a calibrated posterior survives the trip into a weight row.
#
# The failure this exists for is silent. rcarbon labels calMatrix rows in cal BP
# descending - row 1 is the oldest year - while the study grid runs ascending in
# BCE/CE. Reversing the labels without reversing the rows passes every assertion
# downstream and flips the sign of the recovered slope. Nothing errors, the fits
# converge, and the paper is wrong.
#
# So the row is compared against rcarbon's own summary of the same date: the
# peak of the weight row must land on the calibrated median, and the row's own
# median must agree with it.
