...
time_ref_min   <- PERIOD_START - 240   # longest coarse window at the default
time_ref_range <- (PERIOD_END + 240) - time_ref_min

sim <- simulate_typo(N, intercept = 8, slope = 0.02, sigma = 2,
                     period_start = PERIOD_START, period_end = PERIOD_END,
                     seed = 1)

grid <- seq(time_ref_min, time_ref_min + time_ref_range, by = STEP)
